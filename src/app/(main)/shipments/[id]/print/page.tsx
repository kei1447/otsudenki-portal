...
export default async function PrintShipmentPage({ params }: { params: { id: string } }) {
  // パラメータ取得の非同期対応
      ...
      <div className="max-w-[210mm] mx-auto border border-gray-200 p-8 print:border-none print:p-0">
        ...
        <div className="flex justify-between items-end border-b-2 border-gray-800 pb-4 mb-8">
          <div>
            ...
            <p className="text-sm text-gray-600">No. {shipment.delivery_note_number || shipment.id.slice(0, 8).toUpperCase()}</p>
            <p className="text-sm text-gray-600">発行日: {new Date().toLocaleDateString('ja-JP')}</p>
          </div>
          <div className="text-right">
            <h2 className="text-xl font-bold mb-1">{shipment.partners?.name} 御中</h2>
            <p className="text-sm text-gray-500">下記の通り納品いたしました。</p>
          </div>
        </div>
        ...
        <div className="flex justify-between mb-8">
          <div className="w-1/2">
            <div className="border-b border-gray-400 mb-2 pb-1">
              ...
              <span className="ml-4 font-bold text-lg">{new Date(shipment.shipment_date).toLocaleDateString('ja-JP')}</span>
            </div>
            <div className="mt-4">
          ...
          <tbody>
            {shipment.items.map((item: any, index: number) => (
              <tr key={item.id} className="border-b border-gray-300">
                ...
                <td className="py-3 px-2 text-right font-bold">¥ {item.line_total.toLocaleString()}</td>
              </tr>
            ))}
            {/* 空行埋め */}
            {Array.from({ length: Math.max(0, 10 - shipment.items.length) }).map((_, i) => (
