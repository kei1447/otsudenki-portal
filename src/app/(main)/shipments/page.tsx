...
import { getShipmentList, deleteShipment } from '@/app/(main)/inventory/actions'
...
export default function ShipmentListPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('この納品書を削除してもよろしいですか？\n\n・出荷データが完全に取り消されます\n・製品在庫が元の数に戻ります')) return
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-gray-500">納品書データがありません</div>
        ) : (
              ...
              {shipments.map((shipment) => (
                <tr key={shipment.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-500">
                    {shipment.delivery_note_number || shipment.id.slice(0, 8).toUpperCase()}
                  </td>
