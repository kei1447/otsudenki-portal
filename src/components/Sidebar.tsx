  ...
  return (
    ...
    >
      ...
      <div className="flex h-16 items-center justify-between px-4 bg-gray-800 flex-shrink-0">
        {isExpanded ? (
          <span className="text-xl font-bold tracking-wider truncate">Otsu Denki</span>
        ) : (
            ...
            <div className="space-y-1">
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className={`min-w-[1.25rem] w-5 h-5 flex items-center justify-center ${isActive ? 'text-white' : 'text-gray-400'}`}>
        ...
        <div className={`flex items-center ${!isExpanded ? 'justify-center' : ''}`}>
          ...
          {isExpanded && (
            <div className="ml-3 truncate">
              <p className="text-sm font-medium text-white truncate max-w-[10rem]">{user?.email}</p>
              <button onClick={handleSignOut} className="text-xs text-gray-400 hover:text-white mt-1">ログアウト</button>
            </div>
          )}
        </div>
      </div>
