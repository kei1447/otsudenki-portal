...
export default function PriceMatrix({ 
  ...
  const matrixData = useMemo(() => {
    return products.map(product => {
      ...
      const nextPrice = futurePrices.length > 0 
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-bold text-gray-900">
          製品単価リスト (マトリクス)
          <span className="ml-4 text-sm font-normal text-gray-500">{matrixData.length}件 表示</span>
        </h1>
        
        <div className="flex items-center gap-2">
          {canEdit && (
            <>
              <Link href="/prices/bulk" className="rounded-md bg-white px-4 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">一括登録</Link>
              <Link href="/prices/new" className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">新規登録</Link>
            </>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 bg-gray-50 p-4 rounded-md border border-gray-200 items-end">
        <div className="w-full sm:w-auto">
          ...
          <select value={selectedPartner} onChange={(e) => setSelectedPartner(e.target.value)} className="block w-full sm:w-48 rounded-md border-gray-300 py-1.5 text-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            {masterPartners.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
          </select>
        </div>
        ...
        <div className="w-full sm:w-auto flex-1">
          <label className="block text-xs font-medium text-gray-500 mb-1">キーワード検索 (型番・品名・色・備考)</label>
      ...
      <div className="overflow-x-auto rounded-lg border border-gray-200 bg-white shadow">
        <table className="min-w-full border-collapse">
          <thead className="bg-gray-50 select-none border-b border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">取引先</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">型番 / 色</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">品名</th>
              
              {/* ヘッダーの縦罫線を削除し、背景色のみで区分けして被りを解消 */}
              <th className="px-4 py-3 text-right text-xs font-medium text-gray-400 uppercase tracking-wider bg-gray-50">旧単価</th>
              <th className="px-4 py-3 text-right text-xs font-bold text-gray-900 uppercase tracking-wider bg-indigo-50">現行単価</th>
              <th className="px-4 py-3 text-right text-xs font-medium text-blue-600 uppercase tracking-wider bg-blue-50">新単価(予定)</th>
            </tr>
          </thead>
              ...
              return (
                <React.Fragment key={row.id}>
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-sm text-gray-900">{row.partners?.name}</td>
                    <td className="px-4 py-4 text-sm">
                      <div className="font-mono text-gray-900">{row.product_code}</div>
                  ...
                  {isExpanded && (
                    <tr className="bg-gray-50">
                      <td colSpan={8} className="px-4 py-4 border-b border-gray-200 shadow-inner">
                        <div className="ml-10 bg-white rounded border border-gray-200 p-4">
                          <h4 className="text-sm font-bold text-gray-700 mb-2">単価履歴一覧</h4>
                          <table className="min-w-full text-sm">
                            ...
                            <tbody>
                              {row.allPrices.map(p => {
                                ...
                                return (
                                  <tr key={p.id} className={`border-b border-gray-100 ${isCurrent ? 'bg-indigo-50 font-bold' : ''}`}>
                                    <td className="py-2">{p.valid_from} ~</td>
                                    <td className="py-2 text-right">¥{p.unit_price.toLocaleString()}</td>
                                    <td className="py-2 pl-4 text-gray-600">{p.reason || '-'}</td>
                                    <td className="py-2 text-right">
                                      {isCurrent ? <span className="text-indigo-600 text-xs">現行</span> : ''}
                                      {row.nextPrice?.id === p.id ? <span className="text-blue-600 text-xs">予定</span> : ''}
                                    </td>
                                  </tr>
                                )
                              })}
                              {row.allPrices.length === 0 && (
                                <tr><td colSpan={4} className="py-2 text-center text-gray-400">履歴なし</td></tr>
                              )}
                            </tbody>
                          </table>
                          ...
                          {canEdit && (
                            <div className="mt-4 flex justify-end">
                               <span className="text-xs text-gray-400">※修正は「新規登録」または「一括登録」から行ってください</span>
                            </div>
                          )}
                        </div>
                      </td>
        ...
        {matrixData.length === 0 && <div className="p-12 text-center text-gray-500">データがありません</div>}
      </div>
    </div>
  )
}