...
export default async function ProductsPage(props: Props) {
  ...
  // --- 検索条件の取得 ---
  const partnerName = typeof searchParams.partner === 'string' ? searchParams.partner : ''
  ...
  if (keyword) {
    ...
    query = query.or(`product_code.ilike.%${keyword}%,name.ilike.%${keyword}%,color.ilike.%${keyword}%,memo.ilike.%${keyword}%`)
  }
  ...
  if (productsResult.error) {
    return <div className="text-red-500">製品データ取得エラー: {productsResult.error.message}</div>
  }
  if (partnersResult.error) {
    return <div className="text-red-500">取引先データ取得エラー: {partnersResult.error.message}</div>
  }
