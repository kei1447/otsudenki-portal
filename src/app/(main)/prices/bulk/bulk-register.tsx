...
export default function BulkRegister({
  ...
  const handleDownloadTemplate = () => {
    ...
    link.setAttribute(
      'download',
      `price_template_${new Date().toISOString().split('T')[0]}.csv`
    );
  ...
  const parseCSV = (text: string) => {
    ...
    const parsedRows = dataLines
      ...
      .filter(
        (r): r is RowData =>
          r !== null && (r.unitPrice !== '' || r.errorMessage !== '')
      );
  ...
  const handleSubmit = async () => {
    ...
    if (rows.some((r) => r.unitPrice && !r.isValid)) {
      alert(
        'エラーの行が含まれています。修正するか、CSVから削除してください。'
      );
      return;
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden border border-gray-200">
          ...
          <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end">
            <button
              ...
              disabled={
                isSubmitting ||
                rows.some((r) => r.unitPrice && !r.isValid) ||
                rows.every((r) => !r.unitPrice)
              }
      ...
      {resultMsg && (
        <div
          className={`p-4 rounded-md ${resultMsg.type === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'}`}
        >
          {resultMsg.text}
        </div>
      )}
