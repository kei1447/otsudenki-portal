...
import { getRawStockProductsByPartner, getDefectiveProductsByPartner, getRecentShipments, getProductsByPartner } from './actions'

// 型定義
type RawProduct = Awaited<ReturnType<typeof getRawStockProductsByPartner>>[number]
...
export default function OperationPanel({ 
  ...
  // タブ切り替えスタイル
  const tabClass = (id: string) => `px-4 py-2 text-sm font-bold rounded-t-lg transition-colors ${activeTab === id ? 'bg-white text-blue-600 border-t-2 border-blue-600 shadow-sm' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`

  return (
    <div className="space-y-6">
      {/* タブヘッダー */}
      <div className="flex border-b border-gray-200 overflow-x-auto">
        <button onClick={() => setActiveTab('receiving')} className={tabClass('receiving')}>1. 部材受入</button>
        <button onClick={() => setActiveTab('production')} className={tabClass('production')}>2. 加工実績</button>
        <button onClick={() => setActiveTab('shipment')} className={tabClass('shipment')}>3. 出荷登録</button>
        <button onClick={() => setActiveTab('defective')} className={tabClass('defective')}>4. 不良品処理</button>
        <button onClick={() => setActiveTab('history')} className={tabClass('history')}>5. 履歴・取消</button>
      </div>
      ...
      <div className="bg-white p-6 rounded-b-lg border border-gray-200 shadow-sm min-h-[400px]">
        ...
        {activeTab === 'defective' && <DefectiveTab products={defectiveProducts} />}
...
function ReceivingTab({ products }: { products: RawProduct[] }) {
  const [inputs, setInputs] = useState<Record<number, { qty: string, date: string }>>({})
  ...
  const handleRegister = async (p: RawProduct) => {
    ...
    if (confirm(`${p.name} を ${input.qty}個 受入登録しますか？`)) {
      ...
      alert(res.message)
      if (res.success) setInputs(prev => ({ ...prev, [p.id]: { qty: '', date: '' } }))
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-bold text-gray-700 border-l-4 border-yellow-400 pl-3">生地・部材の受入</h3>
      {products.length === 0 ? <p className="text-gray-400 py-4">対象製品がありません</p> : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {products.map(p => (
            <div key={p.id} className="border p-4 rounded bg-gray-50 hover:bg-white transition-colors">
              <div className="font-bold text-lg mb-1">{p.name}</div>
              <div className="text-xs text-gray-500 mb-2">{p.product_code} {p.color}</div>
              <div className="flex justify-between text-sm mb-3">
                <span>現在庫: <span className="font-bold">{p.stock_raw.toLocaleString()}</span></span>
              </div>
              <div className="space-y-2">
                <input 
                  type="number" placeholder="受入数量" 
              ...
              {p.arrivals && p.arrivals.length > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <p className="text-xs font-bold text-gray-400 mb-1">直近の入荷</p>
...
// 2. 加工実績タブ
// ============================================================================
type ProdInput = { finished: string, rawUsed: string, defective: string, defects: Record<string, number>, sourceDate: string }
...
function ProductionTab({ products }: { products: RawProduct[] }) {
  ...
  const handleChange = (id: number, field: keyof ProdInput, value: string) => {
    setInputs(prev => {
      // ★修正: 初期値を外に出してスッキリさせる
      const current = prev[id] || { finished: '', rawUsed: '', defective: '', defects: {}, sourceDate: '' }
      return { ...prev, [id]: { ...current, [field]: value } }
    })
  }
  ...
  const handleDefectsSave = (productId: number, newDefects: Record<string, number>) => {
    setInputs(prev => {
      const current = prev[productId] || { finished: '', rawUsed: '', defective: '', defects: {}, sourceDate: '' }
  ...
  const handleRegister = async (p: RawProduct) => {
    ...
    if (finished + defective === 0) return alert('完成数または不良数を入力してください')
    ...
    if (confirm(`以下の内容で登録しますか？\n\n・製品: ${p.name}\n・良品完成: ${finished}個\n・不良発生: ${defective}個\n・生地消費: ${rawUsed}個\n${inp.sourceDate ? `・入荷日指定: ${inp.sourceDate}` : ''}`)) {
      ...
      alert(res.message)
      if (res.success) setInputs(prev => ({ ...prev, [p.id]: { finished: '', rawUsed: '', defective: '', defects: {}, sourceDate: '' } }))
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-bold text-gray-700 border-l-4 border-blue-500 pl-3">加工実績の登録</h3>
      {products.length === 0 ? <p className="text-gray-400 py-4">対象製品がありません</p> : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {products.map(p => {
             const inp = inputs[p.id] || {}
             return (
              <div key={p.id} className="border p-4 rounded bg-gray-50 hover:bg-white transition-colors relative">
                <div className="font-bold text-lg mb-1">{p.name}</div>
                <div className="text-xs text-gray-500 mb-3">{p.product_code} {p.color}</div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold w-12 text-blue-600">完成</span>
                    <input 
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-gray-200">
                    <span className="text-xs font-bold w-12 text-gray-500">消費</span>
                    <input 
                      type="number" placeholder="自動" 
...
function DefectInputRow({ productId, currentDefects, onSave }: { 
  productId: number, 
  ...
  return (
    <div className="flex gap-6">
      <div className="flex-1 space-y-4">
        <h3 className="font-bold text-gray-700 border-l-4 border-green-500 pl-3">出荷データの登録</h3>
        ...
        {products.length === 0 ? <p className="text-gray-400">登録された製品がありません</p> : (
          <div className="bg-white border rounded shadow-sm overflow-hidden">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  <th className="px-4 py-2 text-left text-xs font-bold text-gray-500">製品コード</th>
                  <th className="px-4 py-2 text-left text-xs font-bold text-gray-500">製品名</th>
                  <th className="px-4 py-2 text-left text-xs font-bold text-gray-500">色</th>
                  <th className="px-4 py-2 text-right text-xs font-bold text-gray-500 w-32">出荷数量</th>
                </tr>
              </thead>
      ...
      <div className="w-64 flex-shrink-0 border-l pl-6 hidden lg:block">
        <h4 className="font-bold text-gray-500 mb-4 text-sm">最近の出荷リスト</h4>
        <div className="space-y-3">
          {history.length === 0 && <p className="text-xs text-gray-400">履歴なし</p>}
          {history.map(h => (
            <div key={h.id} className="border p-3 rounded bg-gray-50 text-sm">
              <div className="flex justify-between text-gray-500 text-xs mb-1">
                ...
                <a href={`/shipments/${h.id}/print`} target="_blank" className="text-blue-600 hover:underline">印刷</a>
              </div>
              <div className="font-bold text-gray-800">¥{h.total_amount.toLocaleString()}</div>
            </div>
          ))}
        </div>
      </div>
...
function DefectiveTab({ products }: { products: DefectiveProduct[] }) {
  const [inputs, setInputs] = useState<Record<number, { rework: string, return: string }>>({})
  const [isSubmitting, setIsSubmitting] = useState(false)

  const handleChange = (id: number, field: 'rework' | 'return', value: string) => {
    setInputs(prev => {
      // ★修正: 初期値を外に出す
  ...
  const handleRegister = async (p: DefectiveProduct) => {
    ...
    if (confirm(`処理を実行しますか？\n\n・手直し良品化: ${rework}個\n・不良品返却: ${ret}個`)) {
      ...
      alert(res.message)
      if (res.success) setInputs(prev => ({ ...prev, [p.id]: { rework: '', return: '' } }))
      setIsSubmitting(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-bold text-gray-700 border-l-4 border-red-500 pl-3">不良在庫の処理</h3>
      <p className="text-sm text-gray-500">
        発生した不良品について、「手直しして良品にする」か「客先へ返却(廃棄)する」かを登録します。
      </p>

      {products.length === 0 ? <div className="p-8 text-center bg-gray-50 rounded text-gray-400">現在、不良在庫はありません</div> : (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {products.map(p => (
            <div key={p.id} className="border p-4 rounded bg-red-50 border-red-100">
              ...
              <div className="mb-4 text-center bg-white rounded p-2 border border-red-200">
                ...
                <span className="text-xl font-bold text-red-600">{p.stock_defective}</span>
              </div>

              {/* 直近の不良理由 */}
              {p.recent_defects.length > 0 && (
                <div className="mb-3 text-xs bg-white/50 p-2 rounded">
                  <p className="font-bold text-gray-500 mb-1">最近の不良理由:</p>
                  {p.recent_defects.map((d, i) => (
                    <div key={i} className="text-gray-600">・{d.reason} ({d.date})</div>
                  ))}
                </div>
              )}
...
function HistoryTable() {
  ...
  const handleUndo = async () => {
    ...
    if (!confirm(`選択した ${selectedIds.size} 件の操作を「取り消し」ますか？\n\n【注意】\n在庫数が操作前の状態に戻ります。\n(例: 受入を取り消すと在庫が減ります)`)) return
    setIsDeleting(true)
  ...
  const getTypeLabel = (type: string) => {
    switch(type) {
      ...
      case 'shipping_cancel': return <span className="bg-gray-600 text-white px-2 py-0.5 rounded text-xs font-bold">出荷取消</span>
      default: return type
    }
  }
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-center bg-gray-50 p-4 rounded-md border border-gray-200">
        ...
        <div className="flex gap-3 items-center">
          <button onClick={loadHistory} className="text-sm text-gray-500 hover:text-indigo-600 underline mr-2">再読み込み</button>
          <button 
            onClick={handleUndo} 
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="divide-y divide-gray-200 text-sm">{history.map(h=>{
                ...
                return <tr key={h.id} className={isSelected?'bg-blue-50':'hover:bg-gray-50'}>
